# @Time : 2024/4/9 8:06
# @Author : Zeyong Ji
from generate_dataload.datasets import Charades, HMDB51, UCF101,AnimalKingdom,kinetics400
...
class DataManager:
    ...
    def get_dataset(self,mode='train'):
        ...
        if (self.dataset == 'charades'):
            data = Charades(self.path, act_dict, total_length=self.total_length,random_shift=False, mode=mode,generate=self.generate)
        elif (self.dataset == "hmdb51"):
            data = HMDB51(self.path, act_dict, total_length=self.total_length, file_number=self.file_number,mode=mode,generate=self.generate)
        elif (self.dataset == "ucf101"):
            data = UCF101(self.path, act_dict, total_length=self.total_length, file_number=self.file_number,mode=mode,generate=self.generate)
        elif (self.dataset == "animalkingdom"):
            data = AnimalKingdom(self.path, act_dict, total_length=self.total_length,random_shift=False, mode=mode,generate=self.generate)
        elif (self.dataset == "kinetics400"):
            data = kinetics400(self.path, act_dict, total_length=self.total_length, random_shift=False, mode=mode,generate=self.generate)
        else:
            raise Exception("[ERROR] The dataset " + str(self.dataset) + " is not supported!")
