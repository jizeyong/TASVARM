...
def main(args):
    ...
    working_dir = connect_path([
        env['output_path'],'few', config['data']['dataset'], str(config['expt']['few']),
        config['iterate']['model'], config['pretrain']['image_mode'], fixed_time])
    ...
    train_loader = manager.get_few_data_loader(train_transform, mode='train')
    logger.info("Train size:" + str(len(train_loader.dataset)))

    # val or test data
    val_transform = manager.get_transforms(mode='test')
    val_loader = manager.get_few_data_loader(val_transform, mode='test')
    ...
    executor.few_run(train_loader, val_loader, logger)
...
def get_parser():
    ...
    # parser.add_argument('--config', '-cfg', type=str, default='./configs/hmdb51/hmdb51_vitb16_few.yaml', help='global config file')
    parser.add_argument('--config', '-cfg', type=str, default='./configs/ucf101/ucf101_vitb16_few.yaml',help='global config file')
